...
def evaluate(data, model, name):
    ...
    for batch_id in range(total_batch):
        ...
        # print("tag:",tag_seq)
        hpred_label,lpred_label, hgold_label,lgold_label = recover_label(B2HH_tag_seqs,B2HB_tag_seqs,batch_hlabel, batch_llabel, mask, data.hlabelset,data.llabelset, batch_wordrecover)
    ...
    B2HH_evals, B2HB_evals, B2H_evals = get_ner_fmeasure(hgold_results, lgold_results, hpred_results, lpred_results, )

    logger.info(
        "DEV --HIGH layer: B2H MODEL  acc:%.4f , p: %.4f, r: %.4f, f: %.4f." %
        (B2HH_evals[0], B2HH_evals[1], B2HH_evals[2], B2HH_evals[3]))

    logger.info(
        "DEV --BOT layer: B2H MODEL  p: %.4f, r: %.4f, f: %.4f ." %
        (B2HB_evals[0], B2HB_evals[1], B2HB_evals[2]))
...
def train(args,data,model):
    ...
    for idx in range(args.iteration):
        ...
        for batch_id in range(total_batch):
            ...
            instance_count += 1
            B2HB_loss,B2HH_loss,B2HB_tag_seqs,B2HH_tag_seqs = model.calculate_loss(batch_word, batch_wordlen, batch_char, batch_charlen, batch_charrecover, batch_hlabel,batch_llabel, mask)

            #todo change to evaluate both layer tag....
            right, whole = predict_check(B2HH_tag_seqs,B2HB_tag_seqs,batch_hlabel, batch_llabel, mask)
            sample_right_token += right
            sample_whole_token += whole
            # print("loss:",loss.item())

            loss = B2HB_loss + B2HH_loss
        ...
        # continue
        B2H_evals, _ = evaluate(data, model, "dev")
        ...
        logger.info(
            ...
            (B2H_evals[0], B2H_evals[1], B2H_evals[2], best_dev))
...
if __name__ == '__main__':
    ...
    if status == 'train':
        ...
        dev_finish = time.time()
        logger.info(
            "DEV --ALL layer: B2H MODEL  p: %.4f, r: %.4f, f: %.4f " %
            (B2H_evals[0], B2H_evals[1], B2H_evals[2]))
